...
- **入口（知识金字塔序，D-16）**：结论层 [DECISIONS.md](DECISIONS.md)（裁定台账 D-01..D-45，终裁 2026-07-27）→ 规范层 [METHOD.md](METHOD.md)。立项与三堂会审全史在 `memory/`（考古层，指针驱动、默认不通读）；前史见 `memory/prelude/`（冻结只读）。
