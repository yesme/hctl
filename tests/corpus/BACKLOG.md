...
> **接线（fix-forward / terminal #38）**：`run.sh` 冻结 manifest；wire 案真实调用 `hctl`（#3/#6–#9/#11–#12/#28）。known-event 结构以 Go 为唯一执行者（#6 table-driven plant）；pure helper 仅 envelope/unknown-type（KNOWN_TYPE_DEFER）。#7 receipt 负例须命中声明 guard 子串（codex-27k §2.2）。
